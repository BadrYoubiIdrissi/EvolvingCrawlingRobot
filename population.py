...
from numpy import inf, floor, pi
...
#        self.updateBest()
    ...
    def updateAverageFitness(self):
        ...
        self.averageRawFitness = ut.average([ind.fitness for ind in self.contenu])
...
#        screen.blit(f, (0,height-500))
        ...
        for i in range(n):
            for j in range(n):
                if k < len(self.especes):
                    e = self.especes[k]
                    ind = e.leader
#                    tesp = f.render("Espece: " + str(self.findEspeces(ind)), True, (0,0,0))
#                    screen.blit(tesp,\
#                                (int(j*xstep+ xstep/2 - tesp.get_width()/2) ,70+ int(i*ystep)))
                    ind.draw((int(j*xstep+ xstep/2) ,150 + int(i*ystep + 40)))
                    l = 0
                    for indi in e.best:
                        indi.draw((int(j*xstep+ xstep/2) ,150 + l * 100 + int(i*ystep + 40)))
                        screen.blit(f.render(str(indi.fitness), True, (0,0,0)), (int(j*xstep+ xstep/2)+ 50 ,150 + l * 100 + int(i*ystep + 40)))
                        l+=1
                    k += 1
